...
class PlayState extends MusicBeatState
{
	...
	var arrowMult:Array<Float> = [1, 1, 1, 1];
	...
	override public function create()
	{
		...
		camArrows.bgColor.alpha = 0;

		FlxG.cameras.reset(camGame);
		FlxG.cameras.add(camArrows);
		...
		switch (curStage)
		{
			...
			case 'tenebris':
				var bg = new BGSprite('tennacity_BG', -625, -335);
				add(bg);
		}
		...
		if(curStage == 'tenebris'){
			dadGroup.visible = false;
			gfGroup.visible = false;
			var light = new BGSprite('tennacity_light', -625, -335);
			add(light);
			black = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
			black.cameras = [camHUD];
			black.alpha = 0;
			eye = new BGSprite('eyeball');
			eye.cameras = [camHUD];
			eye.alpha = 0;
		}
	...
	function set_songSpeed(value:Float):Float
	{
		if(generatedMusic)
		{
			for (note in notes)
			{
				var ratio:Float = value / (songSpeed * arrowMult[note.noteData]);
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				if(curSong == 'Tennacity'){
					camArrows.setFilters([new ShaderFilter(wiggleShit.shader)]);
				}
	...
	function startSong():Void
	{
		...
		if(curSong.toLowerCase() == 'tennacity') songLength -= 28 * 1000;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			else
			{
				...
				//babyArrow.cameras = [oppCamArrows];
				opponentStrums.add(babyArrow);
			}

			babyArrow.cameras = [camArrows];
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			if(zoomTwn != null) zoomTwn.active = false;
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if(zoomTwn != null) zoomTwn.active = true;
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if(daNote.copyY) {
					if (ClientPrefs.downScroll) {
						daNote.y = (strumY + 0.45 * (Conductor.songPosition - daNote.strumTime) * (roundedSpeed * arrowMult[daNote.noteData]));
						if (daNote.isSustainNote && !ClientPrefs.keSustains) {
							//Jesus fuck this took me so much mother fucking time AAAAAAAAAA
							if (daNote.animation.curAnim.name.endsWith('end')) {
								daNote.y += 10.5 * (fakeCrochet / 400) * 1.5 * (roundedSpeed * arrowMult[daNote.noteData]) + (46 * ((roundedSpeed * arrowMult[daNote.noteData]) - 1));
								daNote.y -= 46 * (1 - (fakeCrochet / 600)) * (roundedSpeed * arrowMult[daNote.noteData]);
								if(PlayState.isPixelStage) {
									daNote.y += 8;
								} else {
									daNote.y -= 19;
							...
							daNote.y += (Note.swagWidth / 2) - (60.5 * ((roundedSpeed * arrowMult[daNote.noteData]) - 1));
							daNote.y += 27.5 * ((SONG.bpm / 100) - 1) * ((roundedSpeed * arrowMult[daNote.noteData]) - 1);
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Zoom Tween':
				...
				if(Math.isNaN(val1)) val1 = defaultCamZoom;
				if(Math.isNaN(val2)) val2 = 0.1;
				if(zoomTwn != null) zoomTwn.cancel();
				zoomTwn = FlxTween.tween(FlxG.camera, {zoom: val1}, val2, {ease: FunkinLua.getFlxEaseByString(value2), onComplete: twn -> {
	...
	public function moveCamera(isDad:Bool)
	{
		if(isDad)
		{
			...
			maxCam = [camFollow.x - 50, camFollow.y + 50, camFollow.y - 50, camFollow.x + 50];
			tweenCamIn();
		}
		else
		{
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			else
			{
				...
				MusicBeatState.switchState(new PopupState());
				FlxG.sound.playMusic(Paths.music('freakyMenu'));
				changedDifficulty = false;
			}
			transitioning = true;
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			if(dadCam){
				if(Std.int(Math.abs(note.noteData)) == 1 || Std.int(Math.abs(note.noteData)) == 2) camFollow.y = maxCam[Std.int(Math.abs(note.noteData))];
				else camFollow.x = maxCam[Std.int(Math.abs(note.noteData))];
			}
			if(note.gfNote) {
				char = gf;
			}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(note.noteType == 'Speed Note'){
				arrowMult[note.noteData] += FlxG.random.getObject([-.25, .25, -.15, .15]);
				speedTwns.push(FlxTween.num(arrowMult[note.noteData], 1, 5, null, num -> {
					arrowMult[note.noteData] = num;
				}));
			}
			...
			case 'tennacity':
				switch(curStep){
					...
					case 1008:
						eye.alpha = 1;
						if(ClientPrefs.flashing){
							FlxFlicker.flicker(eye, 1, 0.15, true);
						}
					case 1024:
						black.alpha = eye.alpha = 0;
				}
		}
	}
	...
	override function beatHit()
	{
		...
		if (camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && curBeat % 4 == 0)
		{
 		...
 		if(curBeat % 4 == 0){
			alt *= -1;
			wiggleShit.waveAmplitude = .045 * alt;
			for (tween in cancelTwn) {
				tween.cancel();
			}
			cancelTwn.push(FlxTween.tween(wiggleShit, {waveAmplitude: 0}, (Conductor.crochet * 2) / 1000, {ease: FlxEase.quartOut}));
		} 
